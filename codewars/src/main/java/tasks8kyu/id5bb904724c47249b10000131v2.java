...
public class id5bb904724c47249b10000131v2 {

    public static int points(String[] matches){
        int score = Arrays.stream(matches)
                .map(match -> match.split(":"))
                .map(match -> new int[]{Integer.parseInt(match[0]), Integer.parseInt(match[1])})
                .mapToInt(match -> match[0] > match[1]? 3 : match[0] == match[1]? 1 : 0)
                .sum();
        return score;
    }
